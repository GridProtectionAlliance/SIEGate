...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ConfigurationTypeScreen : UserControl, IScreen
    {
        ...
        public ConfigurationTypeScreen()
        {
            ...
            try
            {
                // Determine if the SIEGate is installed in this folder (could be stand-alone debug application or service), we check exe
                // instead of config file since config file can remain between installations. Since only the SIEGate can be setup with a
                // XML or web service configuration, we disable these radio buttons for SIEGate Manager only installations.
                string exeFileName = Directory.GetCurrentDirectory() + "\\" + App.ApplicationExe;

                if (!File.Exists(exeFileName))
                {
                    // If the SIEGate is not be installed user may have chosen to only install the SIEGate Manager on this system,
            ...
            {
                // Not failing if we cannot determine if SIEGate is available...
            }
        }
