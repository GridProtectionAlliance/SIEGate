...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ApplyConfigurationChangesScreen : UserControl, IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            m_siegateManagerLocalCheckBox.IsEnabled = managerOptionsEnabled;
            m_siegateManagerWebCheckBox.IsEnabled = webManagerOptionEnabled;

            // If the options are disabled, they must also be unchecked.
            if (!managerOptionsEnabled)
                m_siegateManagerLocalCheckBox.IsChecked = false;

            if (!webManagerOptionEnabled)
                m_siegateManagerWebCheckBox.IsChecked = false;

            
            m_setupHistorianCheckBox.IsChecked = false;

            // Set up the state object with the proper initial values.
            m_state["applyChangesToService"] = m_siegateServiceCheckBox.IsChecked.Value;
            m_state["applyChangesToLocalManager"] = m_siegateManagerLocalCheckBox.IsChecked.Value;
            m_state["applyChangesToWebManager"] = m_siegateManagerWebCheckBox.IsChecked.Value;
        ...
        // Occurs when the user chooses to apply changes to the SIEGate service.
        private void SIEGateServiceCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = true;
        }

        // Occurs when the user chooses to not apply changes to the SIEGate service.
        private void SIEGateServiceCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = false;
        }

        // Occurs when the user chooses to changes to the local SIEGate Manager application.
        private void SIEGateManagerLocalCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the local SIEGate Manager application.
        private void SIEGateManagerLocalCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = false;
        }

        // Occurs when the user chooses to apply changes to the SIEGate Manager web application.
        private void SIEGateManagerWebCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToWebManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the SIEGate Manager web application.
        private void SIEGateManagerWebCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
