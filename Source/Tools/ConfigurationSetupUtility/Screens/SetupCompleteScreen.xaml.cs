...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class SetupCompleteScreen : UserControl, IScreen
    {
        ...
        private ServiceController m_siegateServiceController;
        ...
        {
            InitializeComponent();
            InitializeSIEGateServiceController();
                ...
                if (m_state != null)
                {
                    try
                    {
                        ...
                        if (migrate)
                        {
                            ...
                            if (oldDataProviderString == SqliteDatabaseSetupScreen.OldDataProviderString)
                        ...
                        // If the user requested it, start or restart the SIEGate service.
...
#else
                                m_siegateServiceController.Start();
#endif
                            }
                            catch
                            {
                                MessageBox.Show("The configuration utility was unable to start SIEGate service, you will need to manually start the service.", "Cannot Start Windows Service", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }

                        // If the user requested it, start the SIEGate Manager.
                    ...
                    {
                        if (m_siegateServiceController != null)
                            m_siegateServiceController.Close();
                    }
                }
        ...
        public Dictionary<string, object> State
        {
            ...
            set
            {
                ...
                if (Convert.ToBoolean(m_state["restarting"]))
                    m_serviceStartCheckBox.Content = "Restart the SIEGate";
            }
        }
        ...
        // Initializes the SIEGate service controller.
        private void InitializeSIEGateServiceController()
        {
            ServiceController[] services = ServiceController.GetServices();
            m_siegateServiceController = services.SingleOrDefault(svc => string.Compare(svc.ServiceName, "SIEGate", true) == 0);
        }

        // Initializes the state of the SIEGate service checkbox.
        private void InitializeServiceCheckboxState()
        {
#if DEBUG
            bool serviceInstalled = File.Exists(App.ApplicationExe);
#else
            bool serviceInstalled = m_siegateServiceController != null;
#endif
            m_serviceStartCheckBox.IsChecked = serviceInstalled;
            m_serviceStartCheckBox.IsEnabled = serviceInstalled;
        }

        // Initializes the state of the SIEGate Manager checkbox.
        ...
        private void ValidateGrafanaBindings()
        {
            ...
            {
                string archiveBinding = string.Format(GrafanaArchiveBinding, "stat", "http.rest://localhost:6354/api/grafana");
            ...
            if (setupHistorian)
            {
                ...
                {
                    string archiveBinding = string.Format(GrafanaArchiveBinding, historianAcronym, "http.rest://localhost:6454/api/grafana");
